...
func distance(action int) float64 { // количество метров в километре.
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	AverageValueMeanSpeed2 := meanSpeed(action, duration) * kmhInMsec
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(AverageValueMeanSpeed2, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	return (swimmingMeanSpeed(lengthPool, countPool, duration) + float64(swimmingCaloriesMeanSpeedShift)) * float64(swimmingCaloriesWeightMultiplier) * weight * duration
}
