...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	runKmH := meanSpeed(action, duration)
	runCcal := ((runningCaloriesMeanSpeedMultiplier * runKmH * 1.79) * weight / mInKm * duration * minInH)
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	walkCcal := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedMinSec, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return walkCcal
}
...
func SwimmingSpentCalories(lenghtPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	meanSwim := swimmingMeanSpeed(lenghtPool, countPool, duration)
	swimCcal := (meanSwim + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return swimCcal
}
