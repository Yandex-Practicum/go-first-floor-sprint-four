...
func walkingSpentCalories(action int, duration, weight, height float64) float64 {
	weightNorm := walkingCaloriesWeightMultiplier * weight
	physicalCoef := walkingSpeedHeightMultiplier * weight / height * 100
    return (weightNorm + math.Pow(meanSpeed(action, duration) * kmhInMsec, 2) * physicalCoef) * duration * minInH
}
...
func swimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	avgCalories := swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift
    return avgCalories * swimmingCaloriesWeightMultiplier * weight * duration
}
